...
class RecentChatsBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<RecentChat>>(
      stream: locator<IChatLocalStorage>().getRecentChatStream(),
...
class HomePageContactsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      future: locator<IChatLocalStorage>().getWhatsAppContacts(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return Container();
        }
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      onTap: () async {
        ...
        if (group != null) {
          locator<IChatLocalStorage>().setAllReaded(group!.id!);
        } else {
          locator<IChatLocalStorage>()
              .setAllReaded(getChatId(user!.id, chat!.user!.id));
        }
      },
      leading: isGroup!
          ? CircleAvatar(
