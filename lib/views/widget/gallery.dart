...
class GalleryController extends GetxController {
  ...
  Future<void> prepareAttachments(
    ...
    // Récupérer les bytes et le nom de chaque asset sélectionné
    final pickedFiles = await Future.wait(
      selectedAssets.map((e) async {
        final file = await e.asset.file;
        final bytes = await file?.readAsBytes();
        return PickedFileResult(
          fileName: file?.path.split("/").last ?? e.asset.id,
          bytes: bytes ?? Uint8List(0),
        );
      }),
    );

    final attachments = chatController.createAttachmentsFromFiles(pickedFiles);
    ...
    if (shouldReturnFiles!) {
      ...
      Navigator.pop(context, pickedFiles.first);
      return;
    }
