...
import 'package:visitor_pass/views/widget/progress_btn.dart';
...
class _AttachmentPreviewState extends State<AttachmentPreview> {
  bool _doesAttachmentExist() {
    // On considère que la pièce jointe existe si attachment.file (Uint8List) est non null
    return widget.message.attachment!.file != null;
  }
...
class _AttachedImageVideoViewerState extends State<AttachedImageVideoViewer> {
  ...
  @override
  void initState() {
    final self = _chatController.self;
    final other = _chatController.other;
    final group = _chatController.group;
    final clientIsSender = widget.message.senderId == self.id;
    sender = clientIsSender
        ? "You"
        : group != null
            ? group.name!
            : other!.name;
  ...
  Future<void> navigateToViewer() async {
    ...
    final focusNode = _chatController.state.value.fieldFocusNode;
    focusNode.unfocus();
    ...
    return Stack(
      ...
      children: [
        ...
        if (!widget.doesAttachmentExist) ...[
          Text("====!widget.doesAttachmentExist"),
...
class _AttachedVoiceViewerState extends State<AttachedVoiceViewer> {
  ...
  void initState() {
    ...
    playerStateListener = player.onPlayerStateChanged.listen((event) async {
      if (!kIsWeb && event == PlayerState.completed) {
    ...
    if (clientIsSender) {
      ...
      final other = _chatController.other;
      final group = _chatController.group;
      avatarUrl = group != null ? "group_url" : other!.avatarUrl;
    }
  ...
  Future<Duration> getDuration() async {
    ...
    if (file is Uint8List) {
      // Pour le web, utiliser les bytes directement
      await player.setSourceBytes(file);
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: ScreenUtil().screenWidth > 640 ? 0.3.sw : null,
      ...
      onTap: () async {
        ...
        if (file is Uint8List) {
          ...
          if (kIsWeb) {
            ...
            // TODO: Implémenter le téléchargement de fichier pour le web
            // Cela nécessiterait l'utilisation de dart:html ou une autre approche
          }
        } else {
          // Pour mobile, utiliser OpenFile
...
class _DownloadingAttachmentState extends State<DownloadingAttachment> {
  ...
  void initState() {
    ...
    //       DownloadService.instance.getDownloadStream(widget.message.id);
    //   if (downloadStream == null) {
    download();
    // }
    // }

    clientIsSender = _chatController.self.id == widget.message.senderId;
  ...
  Future<void> download() async {
    ...
    await _chatController.downloadAttachment(
      widget.message,
      () {
        widget.onDone();
        setState(() => isDownloading = false);
        setState(() => haveDownloaded = false);
      },
      () {
        if (!mounted) return;
        setState(() => isDownloading = false);
    ...
    // downloadStream = DownloadService.instance.getDownloadStream(widget.message.id)!;
    setState(() => isDownloading = false);
  }

  Future<void> cancel() async {
    await _chatController.cancelDownload(widget.message);
    setState(() => isDownloading = false);
  }

  @override
  Widget build(BuildContext context) {
    if (!haveDownloaded && isDownloading) {
...
class _UploadingAttachmentState extends State<UploadingAttachment> {
  ...
  Future<void> upload() async {
    await _chatController.uploadAttachment(widget.message, "");
  ...
  //   await _chatController.stopUpload(widget.message);
