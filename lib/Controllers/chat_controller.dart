...
class ChatController extends GetxController {
  ...
  void addContactsGroup() async {
    filteredContacts.value = await locator<IChatLocalStorage>().getContacts();
    contacts.value = await locator<IChatLocalStorage>().getContacts();
  }
        ...
        for (var conversation in event) {
          if (conversation.is_group ?? false) {
            if (conversation.messages.isEmpty) {
              locator<IChatLocalStorage>().addMessage(
                message: Message(
                  group: Group(
                    id: conversation.id,
                    ...
                    senderId: prefs.getString("user-id"),
                  ),
                  id: const Uuid().v4(),
                  content: ".",
                  senderId: prefs.getString("user-id")??"",
              ...
              for (var message in conversation.messages) {
                ...
                locator<IChatLocalStorage>().addMessage(
                  message: Message(
                    ...
                    id: senderId == prefs.getString("user-id")
  ...
  // Correction ENREGISTREMENT AUDIO
  Future<void> onRecordingDone(String? conversationId) async {
    ...
    // TODO: Adapter la récupération des bytes de l'audio selon la plateforme (mobile/web)
    // Exemple mobile :
    // import 'dart:io';
    // final recordedFileBytes = await File(path!).readAsBytes();
    Uint8List recordedFileBytes = Uint8List(0); // À remplacer par la vraie récupération multiplateforme

    final messageId = const Uuid().v4();
    final timestamp = await DateTimeService.instance.getDateTime();
    final ext = path != null ? path.split(".").last : "aac";
    final fileName = "AUD_ ${timestamp.millisecondsSinceEpoch ~/ 1000}.$ext";

    await locator.get<IChatLocalStorage>().addMessage(
      message: Message(
        id: messageId,
        content: "",
        group: group,
        ...
        attachment: Attachment(
          ...
          url: "",
          fileName: fileName,
          fileSize: recordedFileBytes.length,
          fileExtension: ext,
          uploadStatus: UploadStatus.uploading,
          autoDownload: true,
          file: recordedFileBytes,
          samples: samples,
        ),
      ),
  ...
  Future<void> sendMessageNoAttachments(
    ...
    if (isGroup) {
      await locator<IChatLocalStorage>().addMessage(
        message: message,
        conversationId: conversationId,
  ...
  void sendMessageWithAttachments(
    ...
    if ({
      ...
      if (message.group != null) {
        ...
        await locator<IChatLocalStorage>()
  ...
  Future<void> uploadAttachment(Message message, String conversationId) async {
    await UploadService.upload(
      taskId: message.id,
      bytes: message.attachment!.file!,
      fileName: message.attachment!.fileName!,
      path: '/api/v1/upload-file?upload_type=online',
  ...
  Future<void> uploadCompleteHandler(
    ...
    result.fold(
      ...
      (r) async {
        print("res===========2");
        await locator<IChatLocalStorage>().updateMessage(
          message.id,
  ...
  Future<void> compressAttachment(Message message) async {
    final compressedBytes = await CompressionService.instance.compressImageBytes(
      message.attachment!.file!,
    );
    message.attachment!.file = compressedBytes;
    message.attachment!.fileSize = compressedBytes.length;
    // fileExtension et fileName restent inchangés
  }
  ...
  Future<void> createGroupe({
    ...
    var request = await chatRepository.createGroupe(
      name: name,
      creatorId: prefs.getString("user-id") ?? "",
  ...
  Future<List<Attachment>?> pickAttachmentsFromGallery(
    ...
    if (kIsWeb) {
      if (!await hasPermission(Permission.storage)) return null;
      ...
      final attachments = await Future.wait(
        result.files.map((file) async {
          final type = AttachmentType.fromValue(
            lookupMimeType(file.name)?.split("/")[0].toUpperCase() ??
                'DOCUMENT',
          );

          double? width, height;
          if (type == AttachmentType.image) {
            (width, height) = await getImageDimensions(file.bytes!);
          } else if (type == AttachmentType.video) {
            (width, height) = await getVideoDimensions(file.bytes!);
          }
      ...
      List<PickedFileResult>? files = await pickMultimedia();
      if (files == null) {
      ...
      final attachments = await Future.wait(
        files.map((file) async {
          final type = AttachmentType.fromValue(
            lookupMimeType(file.fileName)?.split("/")[0].toUpperCase() ??
                'DOCUMENT',
          );
      ...
      Navigator.pop(key.currentContext!);
      // TODO: Implémenter pickFiles ou utiliser FilePicker directement si besoin
      // TODO: Créer ou remplacer AttachmentMessageSender par un widget existant
      return null;
    }
  }
  ...
  Future<void> pickAudioFiles(BuildContext context, mounted) async {
    ...
    List<PickedFileResult>? files = await pickFiles(type: FileType.audio);
    if (files == null) {
      Navigator.pop(key.currentContext!);
      return;
    }
  ...
  Future<List<Attachment>?> pickDocuments(
    ...
    List<PickedFileResult>? files = await pickFiles(type: FileType.any);
    if (files == null) {
      Navigator.pop(key.currentContext!);
      return null;
    }
  ...
  // Correction createAttachmentsFromFiles :
